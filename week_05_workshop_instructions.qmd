...
```{python}
loans_data = pd.read_csv("data/loans_full_schema.csv")
loans_data
```
...
```{python}
loans = loans_data.drop(columns = ["rownames"])
loans
```
...
demog_info = demog_info.rename(columns = {"emp_title" : "employment_title", "emp_length": "employment_length_yrs"})
demog_info
```
...
credit_vars
credit_vars.to_csv("Credit_Information.csv", index = False)
```
...
prime
prime.to_csv("Prime_Borrower.csv", index = False)
```
...
with pd.ExcelWriter('data/tips_subsets.xlsx') as writer:
...
with pd.ExcelWriter('data/loans_subsets.xlsx') as writer:
    demog_info.to_excel(writer, sheet_name='Demog_info', index=False)
    credit_vars.to_excel(writer, sheet_name='Credit_info', index=False)
